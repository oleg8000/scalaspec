...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row.merge(Row(line.head), Row.fromSeq(line.tail.map(_.toDouble)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def primary(name: String): Boolean =
      name.startsWith("t01") ||
      name.startsWith("t03") ||
      name.startsWith("t11") ||
      name.startsWith("t1801") ||
      name.startsWith("t1803")
    def working(name: String): Boolean = name.startsWith("t05") || name.startsWith("t1805")
    def other(name: String): Boolean = {
      val regexp = """t\d{1,}""".r
      name match {
        case regexp(_*) => true
        case _ => false
    ...
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))((res, name) => {
      if (primary(name))
        res.copy(_1 = res._1 :+ col(name))
      else if (working(name))
        res.copy(_2 = res._2 :+ col(name))
      else if (other(name))
        res.copy(_3 = res._3 :+ col(name))
      else
        res
    })
  }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed
      ...
      .agg(
        round(sum($"primaryNeeds") / count($"primaryNeeds"), 1).as("primaryNeeds"),
        round(sum($"work") / count($"work"), 1).as("work"),
        round(sum($"other") / count($"other"), 1).as("other"))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .orderBy($"working".as[Double], $"age".as[Double], $"sex".as[Double])
  }
}
